...
var alchemy = require('node_alchemy')('612be5e9d82c63b648661079c636abbfc7c6f0db' /*process.env.ALCHEMY_API_KEY /*'API KEY GOES HERE'*/ );
...
var wordsFrequency = require('./wordFrequency');
    ...
    client.get('statuses/user_timeline', {
        ...
        if (!err) {
            ...
            for (let i = 0; i < twLength - 1; i++) {
                eachTweet = twObj.tweets[i].text;
                eachTweet = eachTweet.replace(/[^\w\s]/gi, ' '); //remove all special characters
                eachTweet = eachTweet.replace(/[\s\n\t\r]+/g, ' '); //convert all multi-space to single-space
                eachTweet = eachTweet.split(' ').sort(); //sorting and spliting 

                eachTweet = sw.removeStopwords(eachTweet);
                eachTweet = eachTweet.join(' ');
...
function whatNext() {
    ...
    rl.question(colors.input('\n Enter a number here '), (task) => {
        ...
        if (task && typeof task == 'number') {
            if (task == 1) doWordFrequencyAnalysis(); //do word
            if (task == 2) doSentimentAnalysis(); // do sentiment
            if (task == 3) doEmotionAnalysis(); //do emotion
